    ...
    async def asyncTaskCountSteps(self): 
        ...
        while True: 
            ...
            if newStepsCtr > 0:
                ...
                # How many steps do we actually generate to dispense one marble?
                # We run at 8 microsteps to obtain a full step on the motor. 
                # We can fit 4 notches on the main pulleys, which means we vend 4 marbles per full rotation
                # These are 200 steps for full rotation stepper motors on the NEMA23. 
                # so to go 1/4 rotation we only need 50 steps. multiply that by microsteps of 8 and 
                # 50 * 8 = 400 steps
                # However, we now have a gearing system
                ...
                # so it now takes more steps to get 1 marble. the math is 400 / 0.26 = 1538.46, so round to integer
                # stepsPerMarble = 400
                stepsPerMarble = 1538

                # Get integer value of marbles by modding.
                newIncrementalMarbleCtr = totalStepsCtr // stepsPerMarble  # 50 full steps for one marble * 8 microsteps

                # Figure out our remainder
                newLeftoverStepsCtr = totalStepsCtr - (newIncrementalMarbleCtr * stepsPerMarble)
